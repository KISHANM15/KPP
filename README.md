...
| Area | Highlights |
|------|------------|
| **Navigation** | Primary nav now includes: Home, About, Departments, Academics, Admissions, Placements, Research, **Events**, **Library**, Campus Life, Students, Alumni, Contact. |
...
| **Home** | Full-bleed graduation hero, stats + badges, three image-backed journey cards, moving placement marquee, events feature cards, rotating quotes, research CTA band, announcements. |
...
| **Department detail** | Image hero with **at-a-glance metrics**, then full rubric sections + sticky anchor nav. |
| **Academics** | Hero, UG/PG/CE cards, **tabbed academic pillars**, digital learning / NEP dark band, calendar CTA. |
| **Admissions** | Four-step pathway, **three demo eligibility examples** + **Try in form**, live assistant, **downloadable brochures & key dates** (`public/brochures/*.txt`), FAQ accordion, bright CTA strip. |
...
| **Campus Life** | Photo “bento” grid, tabbed life lenses (Live / Fuel / Move / Express), upgraded colorful day timeline cards, support CTA. |
| **Students** | Quick links, handbook-style resource cards, **localStorage-backed** start-of-term checklist, FAQ accordion. |
...
| **Contact** | Desk cards, **OpenStreetMap** embed, department shortcuts, enhanced form (optional phone, message counter), social strip. |
...
- The contact form is **client-side only** (success message after submit). To persist messages, add **Firebase Firestore**, an email API, or a ticket system and replace the handler in [`src/pages/Contact.tsx`](src/pages/Contact.tsx).
